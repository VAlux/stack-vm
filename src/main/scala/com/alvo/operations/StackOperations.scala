...
object StackOperations:
  ...
  def push[A: Monoid](element: Int): ProgramF[A] = Program.createProgramForStack(PUSH(element)) { stack => vm =>
    vm.setStack(element :: stack)
  }

  def dup[A: Monoid]: ProgramF[A] = Program.createProgramForStack(DUP) { stack => vm =>
    stack match
      case x :: xs => vm.setStack(x :: x :: xs)
      case _       => error("dup action expects an argument").apply(vm)
  }

  def swap[A: Monoid]: ProgramF[A] = Program.createProgramForStack(SWAP) { stack => vm =>
    stack match
      case x :: y :: xs => vm.setStack(y :: x :: xs)
      case _            => error("swap action expects 2 arguments").apply(vm)
  }

  def exch[A: Monoid]: ProgramF[A] = Program.createProgramForStack(EXCH) { stack => vm =>
    stack match
      case x :: y :: xs => vm.setStack(y :: x :: y :: xs)
      case _            => error("exch action expects 2 arguments").apply(vm)
  }

  def put[A: Monoid](index: Int = 0): ProgramF[A] =
    Program.createIndexedProgram(PUT(index))(index).apply { (stack, memory) => vm =>
      stack match
  ...
  def get[A: Monoid](index: Int = 0): ProgramF[A] = Program.createIndexedProgram(GET(index))(index).apply {
    (stack, memory) => vm => vm.setStack(memory(index) :: stack)
  }
  ...
  def branch[A: Monoid](termList1: List[Term], termList2: List[Term])(
    branch1: ProgramF[A]
  )(branch2: ProgramF[A])(implicit context: List[Term] => Program[A]): ProgramF[A] =
    Program.createProgramForStack(IF(termList1, termList2)) { stack => vm =>
      stack match
  ...
  def rep[A: Monoid](termList: List[Term])(body: ProgramF[A])(using context: List[Term] => Program[A]): ProgramF[A] =
    Program.createProgramForStack(REP(termList)) { stack => vm =>
      stack match
  ...
  def loop[A: Monoid](testTerm: List[Term], bodyTerm: List[Term])(test: ProgramF[A])(body: ProgramF[A])(implicit
    context: List[Term] => Program[A]
  ): ProgramF[A] =
    Program.createProgramForStack(WHILE(testTerm, bodyTerm)) { _ => vm =>
      @tailrec
