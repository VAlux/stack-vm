...
object TermProgramIsomorphism {
  ...
  def fromCode[A: Monoid](code: TermList)(implicit context: TermList => Program[A]): ProgramF[A] = {

    def buildHomomorphism(code: TermList): ProgramF[A] = Foldable[List].foldMap(code) {
  ...
  def toCode[A: Monoid](program: ProgramF[A]): TermList = program(id).getProgram.termList
}
