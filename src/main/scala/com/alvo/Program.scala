  ...
  class StackProgramBuilder[A] extends ProgramBuilder[A, SProcessor[A]]: 
    override def apply(terms: List[Term], processor: SProcessor[A], program: Program[A]) = new Program[A] {
      override def getProgram = new Action[VirtualMachine[A]] {
        override val run: Processor[A] = vm => vm.status match
          case Some(_) => vm
          case _ => (program.getProgram.action.run compose processor.run(vm.stack)) (vm) // Status is empty -> we can continue
      }.asTermAction(terms)
  }
  ...
  class MemoryProgramBuilder[A] extends ProgramBuilder[A, SMProcessor[A]]: 
    override def apply(terms: List[Term], processor: SMProcessor[A], program: Program[A]) = new Program[A] {
      override def getProgram = new Action[VirtualMachine[A]] {
        override val run: Processor[A] = vm => vm.status match {
          case Some(_) => vm
          case _ => (program.getProgram.action.run compose processor.run(vm.stack, vm.memory)) (vm) // Status is empty -> we can continue
        }
      }.asTermAction(terms)
  }
...
object Program:
  ...
  def id[A: Monoid]: Program[A] = new Program[A] {
    override def getProgram: TermAction[A] = TermAction(action = new Action[VirtualMachine[A]] {
  ...
  def createProgramForStack[A: Monoid](terms: List[Term])(program: SProcessor[A]): ProgramF[A] = context => new Program[A] {
    override def getProgram: TermAction[A] = TermAction(terms, new Action[VirtualMachine[A]] {
      override val run: Processor[A] = vm => vm.status match {
        case Some(_) => vm
  ...
  def createProgramForMemory[A: Monoid](terms: List[Term])(program: SMProcessor[A]): ProgramF[A] = context => new Program[A] {
    override def getProgram: TermAction[A] = TermAction(terms, new Action[VirtualMachine[A]] {
      override val run: Processor[A] = vm => vm.status match {
        case Some(_) => vm
  ...
  def createIndexedProgram[A: Monoid](terms: List[Term])(index: Int): SMProcessor[A] => ProgramF[A] = program => context => new Program[A] {
    override def getProgram: TermAction[A] = TermAction(terms, new Action[VirtualMachine[A]] {
      override val run: Processor[A] = vm => {
        if (index < 0 || index >= VirtualMachine.memorySize)
          VirtualMachine.error[A](s"index [$index] is out of bounds").apply(vm)
  ...
  given programCompositionInstance[A: Monoid]: Monoid[ProgramF[A]] = new Monoid[ProgramF[A]] {
    ...
    override def combine(f: ProgramF[A], g: ProgramF[A]): ProgramF[A] = context => new Program[A] {
      ...
      override def getProgram: TermAction[A] = TermAction(terms, new Action[VirtualMachine[A]] {
        override def run: Processor[A] = g(context).getProgram.action.run compose f(context).getProgram.action.run
      })
    }
  }
